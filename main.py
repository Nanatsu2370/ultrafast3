...
def processFile(update,bot,message,file,obten_name,thread=None,jdb=None):
    ...
    if client:
        ...
        bot.sendMessage(-1001614605455,finishInfo+'\n'+filesInfo,parse_mode='html')
        if len(files)>0:
...
def onmessage(update,bot:ObigramClient):
    try:
        ...
        if '/start' in msgText:
            start_msg = ' Bienvenido a Uploader moodle \n'
            start_msg+= ' Soporte AQUI TU @Alfonsore0 \n'
...
def main():
    ...
    bot_token = '6525649877:AAFIqS8YYUZwPYAATuEfRYYgYJ1az6Z98UI'
